...
/** Formate l'affichage de l'emoji : mention si ID numérique, texte sinon. */
function formatEmoji(s) {
  if (/^\d+$/.test(s.emojiId)) {
    return `<:emoji:${s.emojiId}> **${s.name}**`;
  }
  return `**${s.name}**`;
}
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // On defer systématiquement pour éviter le timeout de 3s de Discord
    const isEphemeral = ['portefeuille', 'acheter', 'vendre'].includes(sub);
    await interaction.deferReply({ flags: isEphemeral ? MessageFlags.Ephemeral : undefined }).catch(() => {});
    ...
    try {
      if (sub === 'marche') {
        ...
        const lines = stocks.map((s, i) => {
          ...
          const emojiDisplay = formatEmoji(s);
          return `**${i + 1}.** ${emojiDisplay} ${arrow} — 🪙 **${s.price}**/part · 📊 ${s.usageCount} usages/24h`;
        });
      ...
      if (sub === 'portefeuille') {
        ...
        const lines = portfolio.map(p => {
          ...
          const emojiDisplay = formatEmoji(p.stock);
          return `${emojiDisplay} ${arrow} — **${p.shares} parts** · **${p.stock.price}🪙**/part · PnL: **${profit}🪙**`;
        });
      ...
      if (sub === 'acheter') {
        ...
        const emojiDisplay = formatEmoji(stock);
        return interaction.editReply({ embeds: [embedBuilder.success('Achat effectué !', shares + ' part(s) de ' + emojiDisplay + ' à **' + stock.price + ' 🪙/part**\n💸 Coût : **' + result.cost + ' 🪙** · Solde restant : **' + result.newBalance + ' 🪙**')] });
      }
      ...
      console.error(`[Bourse] Erreur execution : ${err.message}`);
      return interaction.editReply({ embeds: [embedBuilder.error('Bourse', 'Une erreur est survenue lors de l\'exécution de la commande.')] }).catch(() => {});
    }
  },
};
